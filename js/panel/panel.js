...
chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
  if (message.type.match("disable-url-option-status")) {
    console.log('panel.js');
    console.log(message.option);
    updatePanelStatus(message.option);
  }
});

/**
 * Add/remove the 'active' class from the panel status DOM
 * 
 * @param {boolean} disabledURLFilter - True when Dr.FLARE is disabled
 */
function updatePanelStatus(disabledURLFilter) {
  const panelStatusDOM = $(".cf-debugger-extension-status");
  if (disabledURLFilter) {
    panelStatusDOM.addClass("active");
  } else {
    panelStatusDOM.removeClass("active");
  }
}

function getCustomOptionsAndUpdatePanelStatus() {
  chrome.storage.local.get("options", function (data) {
    let options = data["options"];
    optionDisableURLFilter = options.disableURLFilterOption;
    updatePanelStatus(optionDisableURLFilter);
  });
}
...
$(document).ready(function () {
  ...
  // Display a warning message if the extension is disabled
  getCustomOptionsAndUpdatePanelStatus();
});
